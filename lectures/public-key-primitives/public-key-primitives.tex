...
\begin{frame}{Trapdoor Functions}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \pause \(I\): determinstic function \(x \xleftarrow[]{} I(sk, x)\) (should be hard w/o \(sk\))
        \end{itemize}
...
\begin{frame}{Trapdoor Key Exchange}
    \begin{tikzpicture}
      ...
      \pause
        \node[draw=none,fill=none,anchor=west, font=\footnotesize] (choice0) at ($(ChallengerA) + (.05,-3.3)$) {\(s \leftarrow F(sk, y) = x\)};
...
\begin{frame}{Diffie-Hellman Security}
    \begin{itemize}
        ...
        \item \pause over a cyclic group \(\mathbb{G}\) it is mathematically hard to compute \(\alpha\) given \(g^\alpha\), where \(g\) is a generate of \(mathcal{G}\)
