...
\begin{frame}{Discrete Log}
    \begin{tikzpicture}
      ...
      \pause
        \draw[->,thick] ($(Adversary)+(0,-2.65)$) -- ($(Challenger)+(0,-2.65)$) node [pos=0.5,above,font=\footnotesize] {\(u\)};
      \pause
        \node[draw=none,fill=none,anchor=west, font=\footnotesize] (bit) at ($(Challenger) + (0,-3.05)$) {Verify \(g^x = h * u^c\)};
      \end{tikzpicture}
\end{frame}
...
\begin{frame}{3 Color Graph Problem}
    \begin{tikzpicture}
      ...
      \pause
        \draw[->,thick] ($(Adversary)+(0,-1.95)$) -- ($(Challenger)+(0,-1.95)$) node [pos=0.5,above,font=\footnotesize] {\(u\)};
      \pause
